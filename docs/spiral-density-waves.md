...
Acceptance combines both metrics with an occupied-cell floor, so a few aligned knots cannot pass. Fixed-seed tests require coherent, covered, resolved, actively star-forming arms for `bang => spiral` at ticks 1000 through 1100 and `irregular => spiral` at ticks 1993 through 2093. Long-run probes cover every scenario through tick 4000. Deterministic browser links expose the same states for visual checks.

The intended timing is scenario-specific. The bang start can establish arms around ticks 500 to 1000. The irregular start remains disturbed longer, then reaches its fixed-seed acceptance window at ticks 1993 through 2093.
