...
Each accretion scan updates a serialized exponential moving average of swallowed gas. Once the hole has grown by at least 20%, the normalized rate remains above 0.00025, and tick 2400 has passed, `bh_accretion` starts a 360-tick quasar episode and emits `QuasarIgnition`. A splitmix-derived bipolar axis is stable for the whole episode and reproducible from the master seed plus episode count. An authoritative 56-tick pulse envelope modulates `quasar_feedback` after ordinary gas forces and before integration. Pulse crests accelerate gas in two narrow cones and deposit radiation in a slightly broader pair, while troughs fall quiet. Radiation then uses the existing dissipation and fountain paths, so feedback can evacuate or reshape the host without deleting gas or metals. A 1200-tick cooldown bounds repeat episodes. Full model and reference-seed acceptance: [quasar-feedback.md](quasar-feedback.md).
