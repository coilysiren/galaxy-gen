...
Each accretion scan updates a serialized exponential moving average of swallowed gas. Once the hole has grown by at least 20%, the normalized rate remains above 0.00025, and tick 2400 has passed, `bh_accretion` starts a 3000-tick quasar episode and emits `QuasarIgnition`. A splitmix-derived bipolar axis is stable for the whole episode and reproducible from the master seed plus episode count. `quasar_feedback` runs after the ordinary gas forces and before integration, accelerating gas in two narrow cones and depositing radiation in a slightly broader pair. Radiation then uses the existing dissipation and fountain paths, so feedback can evacuate or reshape the host without deleting gas or metals. A 1200-tick cooldown bounds repeat episodes. Full model and reference-seed acceptance: [quasar-feedback.md](quasar-feedback.md).
