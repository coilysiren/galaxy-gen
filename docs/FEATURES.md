...
The central black hole acts directly on gas as well as stars. Weak nuclear viscosity removes angular momentum across the inner disk while a compact low-angular-momentum sink accretes slowly, allowing stable nuclear rings to remain visible while feeding the hole. Sustained feeding after at least 20% growth can ignite a brief serialized quasar episode. A pulse train strongly accelerates and irradiates broad gas cones along its seed-stable bipolar axis, visibly redistributing the host and feeding the existing galactic fountain without bypassing the baryon or metal ledgers. See [quasar-feedback.md](quasar-feedback.md).
...
Plain `useState`. On desktop the control panel floats over a full-bleed canvas. On mobile the two share a column, canvas above and a horizontal control bar below, so chrome never covers the sim. Controls: galaxy size (default 500), scenario dropdown (the four start => end pairs), generate / play-pause / reset / record. Seed mass and dt are fixed constants - both retired as config surfaces. Reset rebuilds the current seed at tick zero and drops the spent `t` from the address, distinct from generate, which rolls a fresh universe unless `lock=1` pins the seed. The default view carries no instrumentation: the sim tick renders as a caption under the canvas, the one number `?t=` makes actionable, while the full lifecycle counter table, tick-ms, FPS, single-step, and the camera all sit behind `?debug=1`. A chrome toggle hides the panel entirely and mirrors to `?ui=0`, so a clean frame is a shareable address and a recording can be captured without UI. The toggle rests near-invisible and wakes on pointer movement, so the hidden state is never a trap. No keyboard surface. URL param round-trip (`?seed=&size=&scenario=&lock=&t=&ui=&debug=`) uses `history.replaceState`. Generate cycles to a fresh seed each press unless `lock=1` pins it, while a URL-provided seed is honored for the first generate either way. `t=` makes any moment addressable: pausing or stepping stamps the current sim tick into the URL, and loading a seed+t link auto-generates and fast-forwards to that exact tick. Determinism guarantees the identical frame. u64 seed: `crypto.getRandomValues` for fresh, `BigInt` for paste/validate. `data-wasm-ready` gate. Every E2E-touched element has `data-testid` (load-bearing).

## Recording (`src/js/lib/recorder.ts`)

Client-side capture of a run as an animated GIF or an MP4 - no checkout, ffmpeg, or dev server. A `record` control with `gif` / `mp4` pills samples the render funnel and decimates by sim tick, so a capture of a `?seed=` run is reproducible frame for frame. GIF encodes off the draw path via `gifenc` behind an explicit queue with inline-encode backpressure; MP4 encodes through Mediabunny's `CanvasSource` over WebCodecs, with no main-thread pixel readback and the encoder's own backpressure. The mp4 pill renders disabled where WebCodecs or an H.264 encoder is missing, rather than disappearing. Downloads named for the permalink that reproduces the run. See [recording.md](recording.md).

## Visualization (`src/js/lib/dataviz.tsx`)

Canvas (not SVG) renderer: one `<canvas>` per frame, DPR-aware (clamped 2×). A `?seed=`-derived backdrop (starfield, faint nebulosity from the same gas sprites, galactic-plane band) blits as the screen-fixed opaque base; see [starfield.md](starfield.md). The world renders in a deterministic stellar co-rotating frame, so faster gas sweeps through star-forming regions and leaves newborn stars behind instead of appearing to fire them outward. New stars begin dimly beneath both gas passes and ease into the exposed layer with age; association glow follows the same reveal. Pan + zoom is a dev utility behind `?debug=1` (`data-cam-{tx,ty,zoom}`); without it the view is locked. Layers: nebular gas sprites at fractional physical positions in four tiers (cold blue-violet, warm magenta, H-alpha pink, shock-swept [OIII] teal) split into under- and over-star passes, multiply-composited dust lanes, bound-association glows, stellar-class points, cyan compact remnants, a diffuse phase-mixed stellar halo, and transients for supernova and planetary-nebula shells and gamma-ray jets. Active quasars add bipolar light cones, pulse-synchronized packets, and nuclear glare on their gas-feedback axis. Two radial fades hide the finite boundary, because gas and stars end differently. Stars keep the wide fade (0.88 to 1.32 of the disk radius), matching their genuine two-tier soft halo in the physics. Gas fades to zero at 0.94, inside the confinement radius: gas is sprung at the disk radius, which makes that radius an equilibrium every outward-drifting parcel parks on, and under the star fade that density ridge drew at 0.82 alpha - the brightest ring in the frame sitting exactly on the domain boundary. A screen-space vignette then keeps a wide or short viewport from ending the image on a straight edge. A gravitational-lens post-process adds point-mass deflection, Einstein-ring arclets, an inverted inner image, event-horizon shadow, and photon ring. Gas composites per screen-space block rather than per cell, decoupling frame cost - and gas exposure - from grid resolution. See [perf-rewrite.md](perf-rewrite.md), [co-rotating-frame.md](co-rotating-frame.md), and [quasar-feedback.md](quasar-feedback.md).
...
- Perf probes: `perf-profile` attributes a tick per process; `test-perf` runs the frame and pacing specs on real GPU via `playwright.gpu.config.ts`.
