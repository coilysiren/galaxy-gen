...
## GIF recording (`src/js/lib/gif-recorder.ts`)

Client-side capture of a run as an animated GIF - no checkout, ffmpeg, or dev server. A `record gif` control samples the render funnel, decimates by sim tick, and encodes off the draw path via `gifenc`. Downloads named for the permalink that reproduces the run. See [gif-recording.md](gif-recording.md).
