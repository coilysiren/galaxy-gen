...
The stellar lifecycle is bounded rather than immortal. Light stars fade into temporary remnants, massive stars supernova into neutron stars, and core-collapse-scale birth draws split into deterministic compact binaries. A mature pair emits `NeutronStarMerger`, combines into one remnant, accounts for radiated mass, then emits a causally linked short `GammaRayBurst`. Stars that remain beyond the luminous disk and old compact remnants phase-mix into a serialized diffuse stellar-halo reservoir, preserving their mass without keeping an ever-growing resolved point population.
