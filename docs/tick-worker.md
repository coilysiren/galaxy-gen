...
- `snapshot` with `mass, fracX, fracY, tickMs, tickId, stars, transients, radiation, metallicity, counters, stellarHaloMass, bhMass, gasColdFraction, lensScale`. Per-tick render snapshot: gas mass, sub-cell offsets, per-cell heavy-element fraction, star render packing ([x, y, luminosity, colorIndex, stage, clusterId] per star), event transients ([kind, x, y, ticksAgo, magnitude] per recent Supernova/StarBirth/GammaRayBurst), the coarse radiation field, and live UI values including the cumulative association count. Typed arrays are transferred.
