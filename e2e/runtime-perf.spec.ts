...
// Both ends of a run, because they stress opposite halves: fresh is the
// worker's worst case, mature is the renderer's.
...
test.describe("runtime perf", () => {
  for (const size of [250, 500]) {
    for (const phase of PHASES) {
      test(`live run — size=${size} ${phase.name}`, async ({ page }) => {
        test.setTimeout(600_000);
        // debug=1 renders the tick-ms and fps rows the probe reads back.
        await page.goto(`/?seed=${FIXED_SEED}&size=${size}&debug=1`);
        ...
        const stats = await page.evaluate(async (sampleMs) => {
          ...
          const read = (id: string) =>
            Number(
              (document.querySelector(`[data-testid="${id}"]`)?.textContent ?? "0").replace(
                /,/g,
                ""
              )
            );
